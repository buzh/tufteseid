"""Benchmark: cVAT over one real patch at every candidate grid, under both radius
rules (RVT's r_max in pixels verbatim, and r_max scaled to hold the metre reach).

Reports bytes per pixel and seconds per km2 for each, and writes PNGs to /tmp.
"""
...
SIDE_M = 500
# Dropped from every edge before measuring: the horizon scan reads short where
# its rays leave the grid.
CROP_M = 25


def area_resample(dem, src_res, dst_res):
    """Block mean to an arbitrary cell size."""
...
def run(cx, cy, tag):
    ...
    # 0.5 m is RVT's calibration grid, carried as the reference row.
